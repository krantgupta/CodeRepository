...
app.get('/activetodousers/:id',function(req,res){
    var i =0;
    var userId = req.params.id;
    var todayDate = dateFormat(new Date(),'DD-MON-YYYY');
    var tomorowdate = dateFormat(new Date(new Date().getTime() + 24 * 60 * 60 * 1000),'DD-MON-YYYY');
    var toDoData = {};
    var todoArray = [];
    for(i=0;i<user.length;i++){
            if (user[i].id= userId&&user[i].isActive === true&&(user[i].toDos.targetDate===todayDate||user[i].toDos.targetDate.tomorowdate)){
                     toDoData.id = user[i].toDos.id;
                         toDoData.userId=user[i].userId;
                     toDoData.text=user[i].toDos.text;
                    toDoData.done=user[i].toDos.done;
